...
class TMDBAPIUtils:
    ...
    def get_movie_cast(self, movie_id: str, limit: int = None, exclude_ids: list[int] = None) -> list:
        ...
        """
        conn = http.client.HTTPSConnection("api.themoviedb.org", context=self.ssl_context)
    ...
    def get_movie_credits_for_person(self, person_id: str, start_date: str = None, end_date: str = None) -> list:
        ...
        """
        conn = http.client.HTTPSConnection("api.themoviedb.org", context=self.ssl_context)
        endpoint = f"/3/person/{person_id}/movie_credits?api_key={self.api_key}"
        conn.request("GET", endpoint)
        response = conn.getresponse()
        data = json.loads(response.read().decode())
        credits = [
            credit for credit in data.get("cast", [])
            if (start_date is None or credit.get("release_date", "") >= start_date) and
               (end_date is None or credit.get("release_date", "") <= end_date)
        ]
        return credits
...
if __name__ == "__main__":
    ...
    tmdb_api_utils = TMDBAPIUtils(api_key='732e01689882f67c2031002efb73a5b6')
    ...
    for _ in range(iterations):
        print(f"Processing {len(person_ids)} person(s) in this iteration.")

        for person_id in person_ids:
            # Fetch movie credits for each actor
            try:
                movies = tmdb_api_utils.get_movie_credits_for_person(person_id, start_date="1999-01-01",
                                                                     end_date="1999-12-31")
            ...
            for movie in movies:
                ...
                try:
                    cast = tmdb_api_utils.get_movie_cast(movie['id'], limit=5, exclude_ids=[int(person_id)])
                except Exception as e:
                    print(f"Error fetching cast for movie ID {movie['id']}: {e}")
                    continue

                for member in cast:
                    member_id = str(member['id'])
                    member_name = member['name'].encode("ascii", "ignore").decode("ascii").replace(",", "")

                    graph.add_node(member_id, member_name)
                    graph.add_edge(person_id, member_id)
                    new_person_ids.add(member_id)

        # Update person IDs for the next iteration
        person_ids = list(new_person_ids)
